...
# SecureTheCloud — Phase 6 (Corrected Runtime Version)
...
if not opa_result.get("allow"):
    ...
    await r.incr(tenant_usage_key(tenant_id, period, "policy_denied"))
...
exp_timestamp = now + req.ttl_seconds

# Clean expired entries from index
await r.zremrangebyscore(session_index, 0, now)

pipe = r.pipeline()

await pipe.hset(session_key, mapping=session_record)
await pipe.expire(session_key, req.ttl_seconds)
await pipe.zadd(session_index, {sid: exp_timestamp})

await pipe.execute()

period = current_period()
await r.incr(tenant_usage_key(tenant_id, period, "tokens_issued"))
